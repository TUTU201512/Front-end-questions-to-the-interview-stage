...
第二：`IE`和`Opera` 会清理近期最少使用的`cookie`，`Firefox`会随机清理`cookie`。

第三：`cookie`的最大大约为`4096`字节，为了兼容性，一般不能超过`4095`字节。

第四：安全性问题。如果cookie被人拦截了，那人就可以取得所有的session信息。即使加密也与事无补，因为拦截者并不需要知道cookie的意义，他只要原样转发cookie就可以达到目的了。

第五：有些状态不可能保存在客户端。例如，为了防止重复提交表单，我们需要在服务器端保存一个计数器。如果我们把这个计数器保存在客户端，那么它起不到任何作用。
...
`localStorage`是用于持久化的本地存储，除非主动删除数据，否则数据是永远不会过期的。

###web storage和cookie的区别
---------
1.存放数据大小。`Web Storage`是为了更大容量存储设计的，`Cookie`的大小是受限的。

2.与服务器端通信。每次请求一个新的页面的时候`Cookie`都会被发送过去，这样无形中浪费了带宽。

3.独立的存储空间。`Web Storage`每个域（包括子域）有独立的存储空间，各个存储空间是完全独立的，因此不会造成数据混乱。

4.易用性。`Web Storage`拥有`setItem,getItem,removeItem,clear`等方法，不像`cookie`需要前端开发者自己封装`setCookie，getCookie`。

但是`cookie`也是不可以或缺的：`cookie`的作用是与服务器进行交互，作为`HTTP`规范的一部分而存在 ，
而`Web Storage`仅仅是为了在本地“存储”数据而生。

浏览器的支持除了`IE７`及以下不支持外，其他标准浏览器都完全支持，值得一提的是IE总是办好事，例如IE7、IE6中的`userData`其实
就是`javascript`本地存储的解决方案。通过简单的代码封装可以统一到所有的浏览器都支持`web storage`。
...
--------
    ...
    B：不同点：
    float仍会占据位置，position会覆盖文档流中的其他元素。(文字会围绕float的元素，所以float的元素在文档流还是占有有一席之位的。
...
- `content-box`：让元素维持W3C的标准盒模型。宽度和高度分别应用到元素的内容框。在宽度和高度之外绘制元素的内边距和边框。
...
--------
  ...
  *   可继承的样式： font-size font-family font-weight, text-indent text-align, line-height,color
...
>CSS3新增伪类举例：
    ...
    *其父元素必须有且仅有一个p类型的子元素，可以有其他类型的子元素。
    p:only-child    选择属于其父元素的唯一子元素的每个 <p> 元素。
    *其父元素必须有且仅有一个子元素，且该子元素类型为p元素。
    
    p:nth-child(2)  选择属于其父元素的第二个子元素的每个 <p> 元素。
    *p:nth-child(2)选择器匹配属于其父元素的第2个子元素，且该子元素必须是p元素。
     p:nth-child(odd)选择器匹配属于其父元素的第奇数个子元素且这些子元素必须是p元素。个数从1算起。
...
--------
    ...
    边框：创建圆角边框(border-radius)，向矩形添加阴影(box-shadow)，使用图片来绘制边框(border-image)
    文本效果：文本阴影（text-shadow），自动换行(word-wrap:break-word;)
    转换（transform)
    *transform:rotate(90deg) scale(0.85,0.90) translate(0px,-30px) skew(-90deg,0deg);//旋转,缩放,移动,倾斜
    ...
    多背景(background-size、background-origin)
    增加了更多的CSS选择器  
    线性渐变（gradient）
    CSS3 颜色值rgba 表示方式
...
###对BFC规范的理解？
--------
http://segmentfault.com/a/1190000003827247
...
--------
    ...
    “background- repeat”，“background-position”的组合进行背景定位，background-position可以用数字能精确的定位出背景图片的位置。这样可以减少很多图片请求的开销，因为请求耗时比较长；请求虽然可以并发，但是也有限制，一般浏览器都是6个。对于未来而言，就不需要这样做了，因为有了`http2`。
...
--------
    ...
    * IE6双边距bug:同时为一个块属性元素设置向一个方向偏移和对这个方向进行外边距设置，比如float:left,margin-left:45px;
    在其他浏览器下是显示正常的，在ie6下这个元素的margin-left确是45*2=90px。
    解决方法是给元素设置_display:inline(_这个符号只有ie6会识别);
    ...
    *  渐进识别的方式，从总体中逐渐排除局部。 
     ...
     css
          .bb{
           ...
           background-color:#00deff\9; /*IE6、7、8、9识别*/
    ...
    * 怪异模式问题：漏写DTD声明，Firefox仍然会按照标准模式来解析网页，但在IE中会触发怪异模式。
      为避免怪异模式给我们带来不必要的麻烦，最好养成书写DTD声明的好习惯。推荐的写法：`<doctype html>`
...
--------
    ...
    * 拖放(Drag and drop)  
      ...
      画布(Canvas)
      地理定位(Geolocation)
      Web存储  localStorage 长期存储数据，浏览器关闭后数据不丢失；
               sessionStorage 的数据在浏览器关闭后自动删除
      Web Worker
      服务器发送事件
      表单输入类型color、date、time、email、url、search  
...
http://www.cnblogs.com/flashsky/articles/642720.html
    ...
    线程在执行过程中与进程还是有区别的。每个独立的线程有一个程序运行的入口、顺序执行序列和程序的出口。但是线程不能够独立执行，必须依    存在应用程序中，由应用程序提供多个线程执行控制。 
